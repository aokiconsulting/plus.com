...
1. Supabase の **Authentication → Users → Invite user** で、林社長のメールアドレスを招待
2. 届いたメールのリンクを開く → アプリが開き「新しいパスワード」の画面が出るので設定
3. **最初にログインした人が自動で「管理者」** になります
4. 以後のスタッフは、アプリの「設定 → スタッフと権限 → ＋登録」で名前・メール・権限を登録したうえで、Supabase で同じメールを Invite してください
...
| パスワードを忘れた | ログイン画面の「パスワードをお忘れの方」（メールでリンクが届く） |
...
- 認証は Supabase Auth（メール＋パスワード）。役割（管理者／作成者）は `app_state` 内の `staff` で管理。最初にログインしたユーザーが管理者になります。
