...
| 見積書・請求書 | いまの様式と同じレイアウトで表示。ロゴ・角印つき。担当者名入り。画面のままA4のPDFにして保存・LINE／メールに添付 |
...
- PDFは html2canvas＋jsPDF（CDN）でプレビューをそのまま画像化してA4に配置（少し長い場合は縮小して1枚、それ以上は複数ページ）。読めない環境ではブラウザ印刷に自動フォールバック。
